...
| ◐ | 28 Aug | Dev - VR | Identity | `A6b` **Remove deprecated `TenantAccess` & `TenantUserRole` (cleanup backlog)** — remove dual-writes, seeders, and drop legacy tables | M | 3 | Legacy tables, seeders, and dual-writes removed. `al.auth` is strictly identity; `al.master` sole writer to `ace_master.Grants`. |
...
| 2026-08-25 | Claude | **F2 App shell completed.** Sidebar with grouped collapsible accordions, responsive framework, dynamic breadcrumbs, user profile chevron dropdown, ⌘K command palette, and mounted `TenantSwitcher`. Gated navigation link visibility with permissions via auth context `effectivePermissions` check. | Webapp builds warning-clean. All 90 unit tests green, and e2e playwright suites pass. |
| 2026-08-25 | Claude | **Cascaded themes and audited colors.** Replaced over 90 light-only tailwind styles with F1 design tokens (`bg-surface`, `bg-surface-subtle`, `text-foreground`, `border-border`, etc.) across sidebar, app layouts, headers, tables, and settings pages (Company Details, Sites, Users). Configured dynamic theme `FormsProvider` overrides to automatically style protected forms and calculator wizards for dark/light modes. | Verified live in browser: theme toggle switches correctly. |
| 2026-09-01 | Claude | **`F5` Scope context merged to `development`.** `ScopeProvider` (tenant/company/branch — industry/unit modeled but have no backend data source yet, `me/scopes` stops at branch), `useScope()`, `ScopeSwitcher` mounted in `AppHeader` in place of `TenantSwitcher` (deleted — confirmed by grep that `F3`'s diagnostics code never referenced it directly, so this was not a breaking removal). Every scope transition clears the whole query cache (`queryClient.clear()`, not `invalidateQueries()` — a stale entry would otherwise keep serving under the new scope's heading); `scopeQueryKey()` added as the minimal stand-in for the not-yet-built `F4` key-folding helper, not yet adopted by any real query key. | 431 tests green (2 new). Live-verified with `F2`'s shell and `F3`'s diagnostics both mounted — no console errors beyond the expected no-backend network noise in local dev. |
...
| 2026-09-16 | Claude | **`A14` Effective-permission preview merged across all three repos.** Deliberately did **not** bake provenance into `A8`'s `IEffectiveAccessResolver`/`EffectiveAccess` — that class sits on the hot path of every permission check in the platform, and not every `IPermissionProvider` (an API key, a token's already-resolved permissions) has a grant behind it to explain. Instead added a new, narrow `IPermissionProvenanceResolver` (`al.net.authorization`), implemented by `EfPermissionProvider` alongside its existing interface, sharing one internal computation so the union/deny-wins rule is resolved exactly once rather than risking two answers that could disagree. `Carbon-Atlas-Services` exposes it as `GET /api/v1/master/users/{userId}/effective-permissions`, reusing `/me/effective-permissions`'s response shape (`details[].sources` populated this time) rather than a parallel DTO; role names are joined in from `al.master`'s own `Role` catalogue, since the shared SDK only knows ids. `Carbon-atlas-webapp` adds `features/rbac/`, gated client-side on `usePermission('USER:READ')` matching the route's own permission requirement, using the four Ace users from `AceTenantSeeder.cs` as the fixture the task spec names. | `al.net.packages` 446/446 at merge. `Carbon-Atlas-Services` could only be compile-verified against a throwaway local stub of the new SDK types, never a live build/test — this repo's `Directory.Build.props` deliberately refuses any local-SDK escape hatch, building only against a *published* `al.net.packages` package; full verification there waits on that package actually publishing. `Carbon-atlas-webapp` 172/172, plus a real bug found and fixed during manual browser verification: the preview's query hook was missing `networkMode: 'always'` (the same fix `useEffectivePermissions` already needed), without which the query silently stalls — neither loading nor error state ever shows — when the backend is unreachable, rather than settling. |
| 2026-09-16 | Claude | **`A14` aligned with `A15` after both landed on `al.net.packages` `integration` at once.** A15's parent/sub-tenant "descendant reach" work touched the same `Apply()` loop in `EfPermissionProvider` A14 had just changed, and — separately from the merge conflict itself — provenance had a real gap once both features coexisted: a permission a user holds through a *parent* tenant's descendant-reach grant reported a `GrantId`/`RoleId` that do not exist in the tenant actually being previewed, with nothing saying why (verified directly: wrote a throwaway test reproducing the scenario, confirmed the bad output, deleted the test). Fixed by adding `TenantId`/`IsDescendantReach` to `PermissionSource`, threaded through `Carbon-Atlas-Services`' `GrantExplanation` and into the webapp's provenance list as a "via parent tenant `X`" label; the role-name lookup for a descendant-reach source now reads across the tenant boundary via `ScopeFilterOverride.SuppressTenantIsolation`, the same mechanism `A15`'s own sub-tenant aggregation already uses for this exact kind of legitimate cross-tenant read. | `al.net.packages` 461/461 (2 new tests proving `TenantId`/`IsDescendantReach` for both the descendant-reach case and the ordinary case, added to `A15`'s own `TenantHierarchyPermissionTests` fixture rather than a duplicate one). `Carbon-atlas-webapp` 174/174. A follow-up PR comment also caught a missing sidebar/breadcrumb/⌘K entry for the preview screen (`navigation.config.ts` had the route but no nav item) — fixed, gated on the same `USER:READ` the page and route already use. |
| 2026-09-17 | Dev | **`A9` Tenant & company chooser completed.** Post-login context chooser (`/choose-context` and `TenantChooser`) for multi-tenant and multi-company memberships; automatic single-tenant bypass skipping the screen when membership spans only one tenant; search filtering across tenant name/id/subsidiaries; keyboard navigation (`aria-activedescendant` + Arrow/Enter); enterprise card directory with brand monograms (`getOrgBadge`) and active-session indicator; and company drilldown selection via `useMyScopes`. Atomic tenant context switching via `POST /auth/context/switch`; entire TanStack query cache dropped (`queryClient.clear()`) on switch to prevent cross-tenant data leaks; cross-tab session sync listening to `storage` events across `AUTH_KEYS`; shared post-auth destination sanitization (`resolvePostAuthDestination`) ensuring safe redirects to protected routes; and prefetching accessible tenants query during login to avoid redundant network roundtrips. | Real endpoints wired to `al.auth` (7001) and `al.master` (7002). Unit and integration test suites pass (235 web tests, 225 react tests, type-check and lint clean). |
| 2026-09-17 | Dev - VR | **`A12` Membership management UI completed (`features/identity/`).** Admin interface mounted at `/settings/membership` over `al.master` grants: searchable/filterable DataGrid with KPI summary cards, creation modal with 3-tier hierarchical scope picker (`Company` → `Branch` → `Industry` → `Unit`), application gating dimension, validity windows, and mandatory expiration enforcement for Support grants; audit-compliant soft-revocation and suspension dialogs; user cross-tenant drawer. Gated mutating actions (`Grant Access`, row actions, drawer footer) on `USER:UPDATE` while keeping page and cross-tenant reach inspectable on `USER:READ`, preventing 403s for view-only roles. `toGrant` wire adapter normalizes value objects (`ApplicationId`) and null-scope inheritance. `features/identity/GUIDE.md` updated. | 63 tests across 9 test suites in `features/identity` (523 tests monorepo-wide), TypeScript type-check, lint, formatting, and production build clean. |
...
HTTP by any service. `A12` — Membership management UI (`features/identity/`) mounted at
`/settings/membership` over `al.master` grants, supporting hierarchical scopes, application gating,
support grants, soft-revocation, cross-tenant drawers, and `USER:UPDATE` mutation gating.

**Next:** identity work continues with `A7` (application gating, still in progress). `A8`'s result
is now consumed by `A14` (effective-permission preview, merged 2026-09-16 — see session log) via a
new, separate `IPermissionProvenanceResolver`, not by changing `IEffectiveAccessResolver` itself;
`A12`/`F2`'s nav-permission-gating are still the other tasks expected to call it. `A6b` (retire the
old tables) comes after `A7` — it depends on every remaining reader/writer of
`TenantAccess`/`TenantUserRole` moving to `Grant` first, see `A6b`'s own entry for the specific ones
already found.

**Note on `A14`/`A15` overlap:** both landed on `al.net.packages` `integration` around the same time
and touched the same `EfPermissionProvider` loop — see the 2026-09-16 session log entries for the
merge-conflict resolution and the provenance gap it surfaced (a permission held through `A15`'s
parent-tenant descendant reach needs its *own* tenant id in provenance, not the previewed tenant's).
